...
The slow browser path is not primarily Groth16 arithmetic. The worker caches the raw proving-key bytes, but every proof copies and checked-deserializes the complete key again inside WASM. That repeated decode consumes about three quarters of total proof latency.
...
The WASM request-cache result was measured from an isolated prototype in headless Chrome and cryptographically verified. Native and withdrawal cache results are projections obtained by removing a separately measured phase. Two consecutive baseline calls in one WASM process remained 12.416 s and 12.307 s: the existing JavaScript byte cache does not warm the decoded Rust prover.
...
The recommended first release is therefore: retain decoded prover objects, preload the request prover outside the Send critical path, and replace global lease locks with per-request idempotency/CAS plus bounded provider concurrency. This requires no circuit or trusted-setup change.
...
The worker currently retains a `Uint8Array`, not a decoded `RequestProver`/`WithdrawalProver`. Every invocation transfers/copies 5.65 or 7.50 MB into WASM and performs checked Arkworks deserialization and curve validation.
...
Correctness checks passed a production-key request proof against the production verifying key, seven Chrome proofs were unique and 256 bytes each, the worker passed `node --check`, and the rebuilt WASM/test suite passed. The prototype intentionally covered the high-frequency request key only; eagerly retaining the rare withdrawal key needs a separate memory/product tradeoff.
...
| P0 | Decode/preload request prover during wallet load or while old lease settles | Removes cold decode from first Send; can hide most remaining latency | Flow trace plus phase timing | None |
...
Decoded caching and prewarming should ship before any circuit redesign. They are larger, lower-risk wins, and they make it possible to judge whether a trusted-setup migration is still justified.
