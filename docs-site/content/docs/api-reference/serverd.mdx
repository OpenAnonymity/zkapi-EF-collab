...
| Request | `ApiRequest { client_request_id: uuid, payload: string, payload_hash: Felt252, public_inputs: RequestPublicInputs, proof: { backend: "stwo_cairo", public_output_hash: Felt252, proof: base64 } }` |
...
Production requests carry opaque Stwo proof bytes plus their canonical public-
output hash. Private witness envelopes are available only in builds compiled
with the explicit `dev-witness-envelope` feature. `serverd` recomputes request
and response payload hashes from the exact bytes at its boundary.
