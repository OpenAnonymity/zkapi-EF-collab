...
const source = fs.readFileSync(path.join(__dirname, '../oa-chat/chat/components/ChatArea.js'), 'utf8');
...
test('busy timeline actions acknowledge the click and never mutate messages', async () => {
    for (const [method, messageId] of [
        ['handleRegenerateMessage', 'assistant-a'],
        ['handleResendMessage', 'user-a']
    ]) {
        ...
        const app = {
            ...
            beginSessionMutation: () => null,
            acknowledgeSessionMutationBusy(sessionId) {
                acknowledged.push(sessionId);
                return feedback;
            },
            data: { getSessionMessages() { throw new Error('Busy actions must not touch messages'); } }
        };
        const action = createChatArea(app)[method](messageId, button);
        assert.deepEqual(acknowledged, ['session-a']);
        assert.equal(button.disabled, true, 'the acknowledgment remains visible until the UI feedback finishes');
        releaseFeedback();
        await action;
